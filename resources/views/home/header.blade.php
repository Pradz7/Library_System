...
<nav class="navbar navbar-expand-lg navbar-dark py-3 shadow-sm sticky-top" style="background: rgba(15, 23, 42, 0.95); backdrop-filter: blur(10px);">
  <div class="container">
    ...
    <a class="navbar-brand fw-bold fs-3 position-relative" href="{{ url('/') }}">
    ...
    <div class="collapse navbar-collapse justify-content-between" id="navbarNav">
      <ul class="navbar-nav ms-auto me-4">
        <li class="nav-item mx-2">
          <a class="nav-link position-relative px-3 {{ request()->is('/') ? 'active' : '' }}" href="{{ url('/') }}">
            Home
            <span class="nav-underline"></span>
          </a>
        </li>
        <li class="nav-item mx-2">
          <a class="nav-link position-relative px-3 {{ request()->is('explore') ? 'active' : '' }}" href="{{ url('/explore') }}">
            Explore
            <span class="nav-underline"></span>
          </a>
        </li>
      ...
      <div class="d-flex gap-3 auth-buttons">
        @guest
          <a href="{{ url('/login') }}" class="btn btn-outline-light hover-scale rounded-pill px-4">
            Login
          </a>
          <a href="{{ url('/register') }}" class="btn btn-gradient hover-scale rounded-pill px-4">
            Register
          </a>
        @else
